...
export default function ExpensesPage() {
  ...
  async function handleAdd(e) {
    ...
    setForm({ employee_id: '', amount: '', expense_date: '', category: '', notes: '' });
    setSaving(false);
    load();
  }
  ...
  const months = useMemo(() => Object.fromEntries(monthlyTotals).constructor === Object
    ? monthlyTotals.map(([k]) => k) : [], [monthlyTotals]);
  ...
  if (allowed === false) return <div className="shell"><Sidebar active="expenses" /><div className="main">You are not authorized to view this page.</div></div>;
  ...
  return (
    <div className="shell">
      ...
      <div className="main">
        ...
        <h1 style={{ fontSize: 30, marginTop: 4, marginBottom: 24 }}>Expenses</h1>
        ...
        {/* Per-employee breakdown for selected month */}
          ...
          {expenses.length === 0 ? (
            ...
            expenses.slice(0, 20).map((e) => (
              <div key={e.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 20px', borderBottom: '1px solid var(--line)', fontSize: 13 }}>
                <div>
                  <strong>{e.employee?.full_name || 'Unknown'}</strong>
                  <span style={{ color: 'var(--muted)' }}> · {e.expense_date} {e.category ? `· ${e.category}` : ''}</span>
